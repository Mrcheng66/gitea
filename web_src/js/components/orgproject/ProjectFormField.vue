...
<template>
  <div class="field" :class="{required: field.required, 'org-project-form-wide': field.type === 'long_text' || field.type === 'multi_select' || field.type === 'member_array'}">
    ...
    <select v-else-if="field.type === 'single_select'" :id="inputID()" v-model="value" :required="field.required">
    ...
    <select v-else-if="field.type === 'multi_select'" :id="inputID()" v-model="value" multiple>
      <option v-for="option in field.options" :key="option.key" :value="option.key">{{ option.label }}</option>
    </select>
    <select v-else-if="field.type === 'member'" :id="inputID()" v-model.number="value" :required="field.required">
    ...
    <select v-else-if="field.type === 'member_array'" :id="inputID()" v-model="value" multiple>
      <option v-for="member in members" :key="member.id" :value="member.id">{{ member.full_name || member.name }}</option>
    </select>
    ...
    <input v-else-if="field.type === 'date'" :id="inputID()" v-model="value" type="date" :required="field.required">
    <input v-else-if="field.type === 'date_time'" :id="inputID()" v-model="value" type="datetime-local" :required="field.required">
