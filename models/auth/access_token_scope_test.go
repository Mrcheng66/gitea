...
func TestAccessTokenScope_Normalize(t *testing.T) {
	...
	tests := []scopeTestNormalize{
		...
		{"all", "all", nil},
		{"write:activitypub,write:admin,write:misc,write:notification,write:organization,write:package,write:issue,write:repository,write:user,write:project", "all", nil},
		{"write:activitypub,write:admin,write:misc,write:notification,write:organization,write:package,write:issue,write:repository,write:user,write:project,public-only", "public-only,all", nil},
	}
...
func TestAccessTokenScopeCategoryProjectIsAppended(t *testing.T) {
	assert.Equal(t, AccessTokenScopeCategoryProject, AccessTokenScopeCategory(9))
	assert.Equal(t, AccessTokenScopeCategoryProject, AllAccessTokenScopeCategories[len(AllAccessTokenScopeCategories)-1])
	assert.Equal(t, []AccessTokenScope{AccessTokenScopeReadProject}, GetRequiredScopes(Read, AccessTokenScopeCategoryProject))
	assert.Equal(t, []AccessTokenScope{AccessTokenScopeWriteProject}, GetRequiredScopes(Write, AccessTokenScopeCategoryProject))
}
