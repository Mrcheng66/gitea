...
func TestOrgProjectMigrationRegistered(t *testing.T) {
	migrations := prepareMigrationTasks()
	assert.EqualValues(t, 343, migrations[len(migrations)-1].idNumber)
	assert.EqualValues(t, 344, calcDBVersion(migrations))
}
