...
import (
	...
	orgproject_api "gitea.dev/routers/api/v1/orgproject"
	...
	return func(ctx *context.APIContext) {
		...
		for _, category := range requiredScopeCategories {
			...
			case auth_model.AccessTokenScopeCategoryProject:
				ctx.APIError(http.StatusForbidden, "project endpoints are not available for public-only tokens")
				return
			}
		}
	}
	...
	m.Group("", func() {
		...
		m.Group("/orgs/{org}", func() {
			m.Group("/projects", func() {
				m.Combo("").
					Get(orgproject_api.RequireRead, orgproject_api.ListProjects).
					Post(orgproject_api.RequireEdit, bind(api.CreateOrgProjectOption{}), orgproject_api.CreateProject)
				m.Group("/{slug}", func() {
					m.Combo("").
						Get(orgproject_api.RequireRead, orgproject_api.GetProject).
						Patch(orgproject_api.RequireEdit, bind(api.EditOrgProjectOption{}), orgproject_api.EditProject)
					m.Get("/history", orgproject_api.RequireRead, orgproject_api.ListProjectHistory)
					m.Combo("/repositories/{repo_id}").
						Post(orgproject_api.RequireEdit, bind(api.LinkOrgProjectRepositoryOption{}), orgproject_api.LinkRepository).
						Delete(orgproject_api.RequireEdit, orgproject_api.UnlinkRepository)
				})
			})
			m.Group("/project-config", func() {
				m.Combo("/draft").
					Get(orgproject_api.GetDraftConfig).
					Put(bind(api.UpdateOrgProjectConfigOption{}), orgproject_api.UpdateDraftConfig)
				m.Post("/validate", bind(api.ValidateOrgProjectConfigOption{}), orgproject_api.ValidateConfig)
				m.Post("/publish", bind(api.PublishOrgProjectConfigOption{}), orgproject_api.PublishConfig)
				m.Get("/versions", orgproject_api.ListConfigVersions)
				m.Post("/rollback/{version}", bind(api.RollbackOrgProjectConfigOption{}), orgproject_api.RollbackConfig)
			}, orgproject_api.RequireConfigure)
		}, reqToken(), tokenRequiresScopes(auth_model.AccessTokenScopeCategoryProject), orgAssignment(true), checkTokenPublicOnly())
